...
const getSingleAnnouncement = async (req, res) => {
    try {
        ...
        if (!announcement)
            return res.status(404).send("This announcement doesn't exists.")
...
const postAddAnnouncement = async (req, res) => {
    try {
        ...
        res.status(201).send(newAnnouncement._id)
    } catch (error) {
        // TODO: ERROR MIDDLEWARE
        res.status(500).send(
            error.message ? error.message : 'Error creating announcement.'
        )
    }
}
